...
scoops = len(selected)
if scoops == 0:import streamlit as st

# Baskin-Robbins 스타일 키오스크 - Streamlit 단일 파일 앱 (기프티콘 결제 추가)
# 외부 라이브러리 없이 Streamlit만 사용합니다.
# 한국어(하십시오체) 친절한 안내와 이모지를 포함합니다.
...
gift_code = None
if payment == "기프티콘":
    gift_code = st.text_input("기프티콘 번호를 입력하십시오 (예: 12자리) 🎁")
...
if st.button("결제 진행하기 ✅"):
    ...
    elif payment == "기프티콘" and (not gift_code or len(gift_code) < 8):
        st.error("유효한 기프티콘 번호를 입력해주십시오. 🎁")
    else:
